...
export async function POST(request: Request) {
  ...
  if (!body.roomId || !body.playerId || !body.targetPlayerId) {
  ...
  if (room.master_id !== body.playerId) {
    return NextResponse.json({ error: "Not allowed" }, { status: 403 });
  }

  if (!room.players[body.targetPlayerId]) {
    return NextResponse.json({ error: "Player not found" }, { status: 404 });
  }

  const players = { ...room.players };
  delete players[body.targetPlayerId];

  const nextMasterId =
    room.master_id === body.targetPlayerId
